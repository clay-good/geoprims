...
| `src/lib/prefs.js` | What this browser remembers: the unit profile chosen on a tool, the display mode, and the recent and pinned lists — all in local storage, nothing sent anywhere |
...
| `src/lib/diagrams.js`, `test/diagrams.test.mjs` | Vector diagrams from core values: wind triangle, runway components, CPA, fly-by, sky plot, airspeed dial, traverse sketch, earthwork cross-section, and profiles for descent, approach, vertical curves, the atmosphere, climb, horizon, line of sight, and Fresnel clearance |
