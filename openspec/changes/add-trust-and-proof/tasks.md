...
- [ ] 4.2 Build the "How we got this" panel with substituted formulas, responsive default state, and print expansion; verify the substituted-formula and printed-proof scenarios (done: the panel's "Show your work" section renders the core's trace for the page's worked example, and the substituted-formula scenario is pinned on density altitude, intermediates included: station pressure, the pressure altitude, and the air density, ending at the 7,932 ft the card shows. Density altitude is the first tool to show its work; the rest follow one at a time. Pending: the responsive default state and the printed-proof snapshot)
