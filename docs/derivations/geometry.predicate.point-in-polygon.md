...
- `tools/vectors/gen_pip_planar.py` and `core/crates/gp-geometry/tests/predicate.rs` `planar_point_in_polygon_matches_geos` (1.1.0, planar edges): 300 grid polygons, some with a hole, and 3,600 points on the grid and on half steps, 146 of them exactly on an edge or a corner; with straight edges in longitude and latitude and every side-of-edge test exact (Shewchuk's orientation predicate), the verdicts match GEOS's exact predicates on every point, by both rules. Vectors v023 to v025 add a point on a straight 45° N edge (on the boundary with planar edges, outside with geodesic ones, as the geodesic bows north to 45.11° N) and a pentagram in planar mode
