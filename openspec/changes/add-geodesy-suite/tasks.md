...
- [ ] 4.7 Implement convergence, scale factor, and arc-to-chord correction; verify the UTM convergence scenario and PROJ factors agreement (built: UTM and UPS convergence and point scale; and `geodesy.projection.arc-to-chord`: exact t − T at both ends in UTM or any SPCS83 zone, from the chord bearing and the geodesic azimuth less the convergence; 7 UTM vectors from GeoConvert and GeodSolve, the NGS Manual 5 short-line formula agrees to 0.013″, and TM and LCC zones match a projected step along the geodesic; pending: PROJ agreement of the factors)
