...
fn mgrs_matches_geotrans() {
    ...
    for line in text.lines().filter(|l| !l.starts_with('#')) {
        ...
        let r = call("geodesy.grid-ref.mgrs-forward", &json!({"lat": f(0), "lon": f(1), "precision": PRECISION[d]}));
        if r["result"]["mgrs"] != c[3] {
            bad.push(format!("{line} -> {}", r["result"]["mgrs"]));
            continue;
        }
        let b = call("geodesy.grid-ref.mgrs-inverse", &json!({"mgrs": c[3]}));
        let (la, lo) = (b["result"]["corner_lat"]["value"].as_f64().unwrap(), b["result"]["corner_lon"]["value"].as_f64().unwrap());
...
fn mgrs_invariants() {
    ...
    let pts = [(40.446111, -79.982222), (60.5, 4.5), (78.2, 15.0), (-33.8688, 151.2093), (84.3, 50.0), (-89.9, -135.0), (0.0001, 179.9999)];
    for (lat, lon) in pts {
        let full = call("geodesy.grid-ref.mgrs-forward", &json!({"lat": lat, "lon": lon, "precision": "1m"}));
        let g = full["result"]["mgrs"].as_str().unwrap().to_owned();
        let head = &g[..g.len() - 10];
        let (e, n) = (&g[g.len() - 10..g.len() - 5], &g[g.len() - 5..]);
        for (k, p) in PRECISION.iter().enumerate() {
            let r = call("geodesy.grid-ref.mgrs-forward", &json!({"lat": lat, "lon": lon, "precision": p}));
            ...
            assert!(gap <= 1.5 * size, "{want}: corner {gap} m from the point, square {size} m");
        }
    }
}
