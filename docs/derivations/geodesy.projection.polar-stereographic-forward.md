...
# Polar Stereographic forward (`geodesy.projection.polar-stereographic-forward`)
...
- t(φ) as for the Lambert conic (for the south pole, t(−φ)); K = √((1 + e)^(1+e)(1 − e)^(1−e))
- Variant A: ρ = 2 a k₀ t / K. Variant B: k₀ = m_c K/(2 t_c) from the standard parallel φ_c, then as variant A
- North: E = FE + ρ sin Δλ, N = FN − ρ cos Δλ. South: N = FN + ρ cos Δλ
- Convergence = ±Δλ (+ north, − south); scale h = k = ρ/(a m), and k₀ at the pole

## Symbols and units

`lat`, `lon` in degrees; `pole` N or S (N if not given); for variant B a `standard_parallel` in the pole's hemisphere, or for variant A a `scale_factor` (1 if not given); `longitude_of_origin`, `false_easting` and `false_northing` in any length unit, and the ellipsoid. Out come `easting` and `northing` in meters, `convergence` in degrees, and the two scales, equal because the projection is conformal.

## Domain

Any point except the pole opposite the center, which is at infinity and is refused. A standard parallel on the equator or in the other hemisphere is refused, and so is a standard parallel given together with a scale factor.
...
- independent: yes
- inputs: 73° N, 44° E on UPS North; 75° S, 120° E on the Australian Antarctic grid
- outputs: E = 3,320,416.75 m, N = 632,668.43 m; E = 7,255,380.79 m, N = 7,053,389.56 m
...
- `tools/diff/runner.mjs`: `polar-stereographic-forward`, 10,000 random points on twelve random grids of both variants and both poles against PROJ's `proj +proj=stere`, within 1 µm
