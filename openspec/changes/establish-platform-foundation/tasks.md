...
- [ ] 6.5 Implement the benchmark harness (reference profile, p50/p95, diff against previous release); verify output table and regression failure on a slowed fixture (built so far: the Node runner measures all 205 primary examples with 50 warm-up and 1,000 timed calls per tool, emits a p50/p95 table and JSON report, compares same-profile reports, and fails a slowed fixture above a 20% p95 regression; pending: the 4× Chromium reference run, per-tool budgets, and a previous-release baseline)
