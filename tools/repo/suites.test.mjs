...
test('every test file in the repository is run by a test script', () => {
  ...
  const webScript = JSON.parse(readFileSync(join(root, 'apps/web/package.json'), 'utf8')).scripts.test;
  const patterns = [...globs(rootScript), ...globs(webScript).map((r) => new RegExp(r.source.replace('^', '^apps/web/')))];
  const files = testFiles(root);
  assert.ok(files.length > 10, 'found the test files');
  for (const f of files) {
    assert.ok(patterns.some((p) => p.test(f)), `${f} is not run by "npm run test:js" or the website's "npm test"`);
  }
});
