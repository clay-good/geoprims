...
- [ ] 4.4 Implement progress reporting and cooperative cancellation for long tools; verify cancellation returns within 100 ms in a benchmark fixture (built so far: the web client interrupts a superseded Wasm call by replacing its worker, discards its answer, replays unrelated pending calls, and shows elapsed time every 250 ms; the simulated stuck-call benchmark returns within 100 ms. The Node worker host accepts an AbortSignal, reports elapsed time every 250 ms, and passes a real spinning-worker cancellation benchmark within 100 ms. The MCP stdio server forwards opt-in progress notifications and honors cancellation for running and queued calls. Pending: core-level cooperative cancellation and an actual long-tool browser benchmark)
...
- [x] 7.2 Implement the egress privacy test (sentinel inputs, proxy capture, zero sentinel matches); verify it catches a deliberately leaking fixture (apps/web/test/egress.test.mjs checks both halves. What the shipped code can do: no sendBeacon, XMLHttpRequest, WebSocket, EventSource, or importScripts anywhere in the bundle, every fetch naming a same-origin path, and exactly one POST, to /api/reports. What a sentinel can reach: a typed value and a note appear only in the report the user previewed, an x-private input never appears and withholds the outputs beside it, excluding the inputs leaves no sentinel anywhere including the permalink, and the payload has no field that identifies a person or a device. A fixture that adds a sendBeacon and an absolute fetch to a built bundle fails it. Pending: the proxy capture in a real browser, which waits on the Playwright suites)
