...
| `diff/runner.mjs` | `npm run diff`: 10,000 seeded random and edge-biased cases per family (geodesic inverse and direct, rhumb inverse, UTM, local ENU, EGM96 geoid height) through the shipped Wasm and GeographicLib's command-line tools, compared within declared tolerances, with failing cases written to `dist/diff/`. Families whose reference is not installed are skipped. `diff/runner.test.mjs` runs 300 cases each and checks that a perturbed tool fails |
