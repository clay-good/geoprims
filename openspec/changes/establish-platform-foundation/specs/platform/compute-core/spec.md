...
The core SHALL be split into independently loadable modules by domain (at minimum: `base` (which also serves the `units` domain), `geodesy`, `navigation`, `geometry`, `aviation`, `drone`, `survey`, `indexing`, `raster`, `time`, and `search` (the catalog ranker, loaded when search is first used)). A tool page SHALL load only `base` plus the modules its tool declares. Each domain module SHALL be at most 400 KB compressed (Brotli), and `base` SHALL be at most 120 KB compressed.
