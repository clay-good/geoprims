...
- [x] 3.1 Render OG images at build with content-hash caching; verify the OG-image scenario (apps/web/scripts/og.mjs renders one 1200 × 630 JPEG per page in Chromium after the Astro build, cached by a hash of the card's HTML; a tool's card shows its worked-example answer over contour lines seeded by its path, and preset pages share their canonical page's card. apps/web/test/og.test.mjs checks every indexable page names an existing 1200 × 630 og:image with og:image:alt, and that no build-only data-og-* attribute ships)
