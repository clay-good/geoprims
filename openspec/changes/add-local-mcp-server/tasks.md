...
- [x] 3.1 Implement toolsets (`geodesy-core`, `navigation`, `e6b`, `atmosphere`, `drone-mapping`, `survey-cogo`, `indexing`) and `--no-meta`; verify the e6b and unknown-toolset scenarios (2026-09-24: `navigation` had grown to 46 stable tools and `geodesy-core` to 41, so the tools past 40 were silently left out, a magnetic tool among them; geometry is now its own toolset and State Plane lives in `projections` only, and `mcp/server.test.mjs` fails when any toolset matches more stable tools than its cap)
