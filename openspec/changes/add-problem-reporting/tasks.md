...
- [ ] 1.1 Add the single report button to the tool header component with lazy import; verify the every-tool and nothing-before-click scenarios in end-to-end tests
- [ ] 1.2 Build the dialog (disclosure, payload preview, include toggle, 280-character note with counter, kind selector, accessible live status); verify the preview and exclude scenarios and an axe pass in every theme
- [ ] 1.3 Implement payload construction with versions, build hash, sanitized permalink, `x-private` omission, and display class; verify the no-identifying-fields schema check and the private-input scenario
- [ ] 1.4 Implement paused, offline (copy report), and success states; verify the offline scenario
- [ ] 1.5 Ensure the service worker bypasses `/api/*`; verify with a service-worker test

## 2. Worker and D1

- [ ] 2.1 Create the D1 database and migration `0001_problem_reports.sql` per design R6; verify with `wrangler d1 migrations apply --local` in CI
- [ ] 2.2 Implement the Worker (config endpoint, POST validation order, 24 KB streaming cap, catalog import, uniform 202, security headers, logging disabled); verify the unknown-tool, oversized, and logging scenarios
- [ ] 2.3 Implement the keyed daily reporter hash, attempt and accepted caps, dedupe, and the atomic batch insert; verify the uniform-response and no-address scenarios
- [ ] 2.4 Implement Turnstile siteverify with action and hostname checks and timeout; verify with Turnstile test keys
- [ ] 2.5 Implement the daily cron cleanup; verify the scheduled-cleanup scenario
- [ ] 2.6 Implement the kill switch (503 config when unconfigured); verify the paused state end to end
...
- [ ] 4.4 Create `known-issues.json` schema, the `/known-issues` page, and tool-page banners; verify the banner scenario
