...
fn table() -> Vec<(f64, f64, f64, f64, bool)> {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/horizon_table12.csv");
    std::fs::read_to_string(path)
        ...
        .map(|l| {
            ...
            (c[0].parse().unwrap(), c[1].parse().unwrap(), c[2].parse().unwrap(), c[3].parse().unwrap(), c.len() > 4)
        })
        .collect()
}
...
fn the_rule_matches_bowditch_table_12() {
    ...
    for (feet, nm, statute, meters, misprint) in rows {
        ...
        let rule = r["result"]["rule_visual"]["value"].as_f64().unwrap_or_else(|| panic!("{r}"));
        // The height in meters the table prints beside it is the same height.
        assert!((meters - feet * 0.3048).abs() <= 0.005, "{feet} ft is {meters} m");
        if misprint {
        ...
        assert!((rule - nm).abs() <= 0.05 + 1e-9, "{feet} ft: rule {rule} vs the table's {nm} nm");
        // The statute column is the same rule in statute miles, from the unrounded value.
        let sm = horizon(&format!("{feet} ft"), "mi")["result"]["rule_visual"]["value"].as_f64().unwrap();
        assert!((sm - statute).abs() <= 0.05 + 1e-9, "{feet} ft: {sm} vs the table's {statute} statute miles");
        checked += 1;
    }
...
fn horizon_invariants() {
    ...
    let value = |r: &Value, k: &str| r["result"][k]["value"].as_f64().unwrap_or_else(|| panic!("{k} in {r}"));
    for _ in 0..200 {
        let h = 0.5 + rnd() * 8000.0;
        let r = horizon(&format!("{h} m"), "km");
        let (geometric, optical, radio) = (value(&r, "geometric"), value(&r, "optical"), value(&r, "radio"));
        ...
        assert!((value(&twice, "rule_visual") / value(&r, "rule_visual") - 2f64.sqrt()).abs() < 1e-9);
        // The arc falls short of the square-root scaling, never past it.
        let ratio = value(&twice, "geometric") / geometric;
        assert!(ratio <= 2f64.sqrt() + 1e-12 && ratio > 1.40, "{h} m scaled by {ratio}");
