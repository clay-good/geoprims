...
- [ ] 5.3 Write `server.json` for `com.geoprims/mcp` with DNS verification, npm and MCPB packages, and `fileSha256`; verify with the registry publisher's validation (built: `mcp/server.json` against the registry's 2025-09-29 schema, naming the npm package and the MCPB download of the same release over stdio, with the digest written from the built bundle by `npm run build:mcpb -- --write-digest`; the committed digest is a placeholder because it only exists once the release artifact does. A test holds the name, the description cap, the version agreement with package.json, and the digest loop. Pending: DNS verification and the publisher's own validation, which need the domain and a release)
