...
| `src/assets.mjs` | Verifies dataset digests before providing bytes to a module; Node and browser hosts use the same integrity check. The browser host evicts corrupt cached bytes and stores a verified replacement for offline use; the Node host reads packaged files without a cache |
