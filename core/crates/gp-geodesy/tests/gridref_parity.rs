...
const GEOREF: [&str; 7] = ["15deg", "1deg", "1min", "0.1min", "0.01min", "0.001min", "0.0001min"];
...
const SYSTEMS: [(&str, &str, &str, &str, &[&str], &str); 3] = [
    ("geodesy.grid-ref.gars-forward", "gars", "geodesy.grid-ref.gars-inverse", "gars", &GARS, "gars"),
    ("geodesy.grid-ref.georef-forward", "georef", "geodesy.grid-ref.georef-inverse", "georef", &GEOREF, "georef"),
    ("geodesy.grid-ref.maidenhead-forward", "locator", "geodesy.grid-ref.maidenhead-inverse", "locator", &MAIDENHEAD, "maidenhead"),
];

#[test]
fn grid_references_match_python_libraries() {
    let text = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/gridref_diff.jsonl")).unwrap();
    let mut bad = Vec::new();
    let mut n = 0;
    for line in text.lines().skip(1) {
        ...
        for (fwd, out, inv, field, precs, key) in SYSTEMS {
            for (p, want) in precs.iter().zip(r[key].as_array().unwrap()) {
                ...
                let close = |k: &str, want: &Value| (v(&d, k) - want.as_f64().unwrap()).abs() < 1e-9;
                if !(close("south", s) && close("west", w) && close("lat", clat) && close("lon", clon)) {
    ...
    assert!(n > 7_000 / 2, "{n}");
    assert!(bad.is_empty(), "{} of {n} mismatches:\n{}", bad.len(), bad[..bad.len().min(12)].join("\n"));
}
...
fn spaced(m: &str) -> String {
    ...
    let h = digits.len() / 2;
    [gzd, sq, &digits[..h], &digits[h..]].iter().filter(|s| !s.is_empty()).copied().collect::<Vec<_>>().join(" ")
}
...
fn usng_matches_geotrans_mgrs() {
    ...
    let text = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/mgrs_diff.csv")).unwrap();
    ...
    for line in text.lines().filter(|l| !l.starts_with('#')) {
        ...
        let r = call("geodesy.grid-ref.usng-forward", &json!({"lat": f(0), "lon": f(1), "precision": USNG[d]}));
