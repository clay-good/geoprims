...
fn metar_matches_python_metar() {
    ...
    for line in text.lines().skip(1) {
        ...
        let r: Value = serde_json::from_str(&REGISTRY.invoke("aviation.weather.metar-decode", &json!({"report": report}).to_string())).unwrap();
        let got = &r["result"];
        ...
        miss("wind speed", v("wind_speed"), want["wind_speed"].as_f64(), 1e-5);
        miss("wind direction", v("wind_direction"), want["wind_direction"].as_f64(), 0.0);
        miss("gust", v("wind_gust"), want["wind_gust"].as_f64(), 0.0);
        miss("visibility (m)", v("visibility").map(|x| x * MI_M), want["visibility_m"].as_f64(), 1.0);
        miss("temperature", v("temperature"), want["temperature"].as_f64(), 1e-9);
        miss("dew point", v("dew_point"), want["dew_point"].as_f64(), 1e-9);
        let alt = got["altimeter"]["value"].as_f64().map(|a| if got["altimeter"]["unit"] == "inHg" { a * INHG_HPA } else { a });
        miss("altimeter (hPa)", alt, want["altimeter_hpa"].as_f64(), 0.01);
        miss("sea-level pressure", v("sea_level_pressure"), want["sea_level_pressure"].as_f64(), 1e-9);
        let ours: Vec<(String, Option<f64>)> = got["clouds"]
            .as_array()
            .map(|a| a.iter().map(|c| (cover_code(c["cover"].as_str().unwrap()).to_owned(), c["base"]["value"].as_f64())).collect::<Vec<_>>())
            .unwrap_or_default()
            .into_iter()
