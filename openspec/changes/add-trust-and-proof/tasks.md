...
- [ ] 3.1 Write the derivation template and one derivation per hero tool first; verify the missing-derivation gate (template and gate done in tools/trust/promotion.mjs; notes written for the twenty-four stable tools in docs/derivations/, including the pilot hero tools pressure altitude, density altitude, runway components, and the wind triangle, and the developer hero tools geohash, tile from point, tile bounds, H3 grid disk, and haversine, the drone GSD and height-for-GSD tools, and the surveyor traverse closure and combined factor; pending: the remaining hero tools, tracked in docs/launch/hero-tools.md)
