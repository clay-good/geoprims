...
# NBR (`raster.index.nbr`)

## Method

Healthy vegetation reflects near-infrared strongly and shortwave infrared weakly; burned ground reverses that, so the normalized burn ratio drops sharply after a fire.
...
Inputs: `nir`, `swir2`, surface reflectance of the near-infrared and shortwave infrared 2 (about 2.2 µm) bands, 0 to 1 (unitless). Output: `nbr`, unitless.
...
- sourceTitle: spyndex, the Python front end of the Awesome Spectral Indices catalog; the index as catalogued: NBR, from Key and Benson (2006), FIREMON
- sourceEdition: spyndex 0.12.0
- sourceLocator: `spyndex.computeIndex("NBR", params)`, with the catalog's constants
- independent: yes
