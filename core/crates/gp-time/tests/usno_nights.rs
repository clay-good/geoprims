...
fn nights_follow_usno() {
    let text = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/tests/data/usno_nights.csv")).unwrap();
    ...
    for line in text.lines().filter(|l| !l.starts_with('#')) {
        ...
        let mut expect = |key: &str, want: Option<(i64, i64)>| {
            ...
            match window(r["result"][key].as_str().unwrap_or("")) {
                ...
                got => bad.push(format!("{line}: {key} {got:?} vs USNO ({a}, {b}) from {}", r["result"][key])),
            }
        };
        expect("position_lights", set.zip(rise));
        expect("logging_night", dusk.zip(dawn));
        expect("passenger_currency", set.zip(rise).map(|(s, r)| (s + 60, r - 60)));
