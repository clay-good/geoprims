...
QFE = p(elev + h(QNH)), the altimeter-setting relation (an altimeter set to
QNH reads the elevation on the field); the NWS formula from its published PDF; the US
...
def flight_level():
    ...
    for qnh, fl, alt in [(29.42, 185, None), (29.92, 180, None), (30.25, None, 17500), (28.50, 200, 19000), (27.00, 350, None), (29.00, 450, 12000)]:
        qnh_pa = qnh * INHG
        # On QNH the altimeter reads the pressure altitude less h(QNH).
        offset = h_for(qnh_pa)
        exp = {"result.lowest_usable_fl": lufl(qnh), "result.transition_altitude.value": 18000.0}
        inp = {"altimeter": f"{qnh} inHg"}
        if fl is not None:
            inp["flight_level"] = fl
            exp["result.altitude_of_fl.value"] = (fl * 100 * FT - offset) / FT
        if alt is not None:
            inp["altitude"] = f"{alt} ft"
            exp["result.fl_of_altitude"] = (alt * FT + offset) / FT / 100
