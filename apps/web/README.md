...
| `src/lib/import.mjs`, `src/lib/import-rows.mjs`, `test/import*.test.mjs` | Reading a file a reader brings — GeoJSON, KML, GPX, WKT, CSV — in the worker, never fetching what it points at, repairing geometry with a report; and filling a tool's list of points from it, the one place a pair's order is decided |
