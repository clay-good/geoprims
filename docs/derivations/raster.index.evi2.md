...
# EVI2 (`raster.index.evi2`)

## Method

EVI2 is a two-band form of EVI for sensors without a blue band, fitted to follow EVI closely where aerosols are low.
...
- EVI2 = G · (NIR − Red) / (NIR + 2.4·Red + L), with G = 2.5, L = 1

## Symbols and units

Inputs: `nir`, `red`, surface reflectance of the near-infrared and red bands, 0 to 1 (unitless). Output: `evi2`, unitless.
...
- sourceTitle: spyndex, the Python front end of the Awesome Spectral Indices catalog; the index as catalogued: EVI2, from Jiang, Huete, Didan, and Miura (2008), Remote Sensing of Environment
- sourceEdition: spyndex 0.12.0
- sourceLocator: `spyndex.computeIndex("EVI2", params)`, with the catalog's constants
- independent: yes
...
- `core/crates/gp-raster/tests/indices.rs` `index_invariants`: EVI2 has the same sign as NDVI on the same bands.
