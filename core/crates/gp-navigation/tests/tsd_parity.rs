...
fn table() -> Vec<(f64, f64, f64, bool)> {
    ...
    std::fs::read_to_string(path)
        ...
        .map(|l| {
            ...
            (c[0].parse().unwrap(), c[1].parse().unwrap(), c[2].parse().unwrap(), c.len() > 3)
        })
        .collect()
}
...
fn distance_matches_bowditch_table_11() {
    ...
    for (minutes, knots, miles, misprint) in rows {
        ...
        let got = r["result"]["distance"]["value"].as_f64().unwrap_or_else(|| panic!("{r}"));
        ...
        worst = worst.max(gap);
        checked += 1;
        assert!(gap <= 0.05 + 1e-9, "{minutes} min at {knots} kt: {got} vs the table's {miles}");
    }
...
fn every_unknown_is_solved_the_same_way() {
    ...
    for (minutes, knots, _, misprint) in table().into_iter().filter(|r| r.0 as u32 % 7 == 3) {
        ...
        let d = by(json!({"speed": format!("{knots} kt"), "time": format!("{minutes} min"),
                          "options": {"outputUnits": {"distance": "NM"}}}));
        assert!((d["result"]["distance"]["value"].as_f64().unwrap() - exact).abs() < 1e-9);
        let s = by(json!({"distance": format!("{exact} NM"), "time": format!("{minutes} min"),
                          "options": {"outputUnits": {"speed": "kt"}}}));
        assert!((s["result"]["speed"]["value"].as_f64().unwrap() - knots).abs() < 1e-9, "{s}");
        let t = by(json!({"distance": format!("{exact} NM"), "speed": format!("{knots} kt"),
                          "options": {"outputUnits": {"time": "min"}}}));
        assert!((t["result"]["time"]["value"].as_f64().unwrap() - minutes).abs() < 1e-9, "{t}");
    }
}
