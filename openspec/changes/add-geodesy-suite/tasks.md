...
- [ ] 3.5 Implement NADCON5 grid transformations with chaining; verify against NCAT outputs for 200 points per region and the outside-grid scenario (built: the CONUS NAD 27 ↔ NAD 83 (1986) grid as an on-demand asset with NADCON5's biquadratic interpolation, matching PROJ's gridshift to 1e-9° on 22 points, and the outside-grid scenario; pending: the later CONUS links (HARN, FBN, 2007, 2011, about 95 MB), the other regions, per-point accuracy grids, and NCAT comparison)
